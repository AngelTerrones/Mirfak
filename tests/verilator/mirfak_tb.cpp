...
#define dwbm_addr_o m_core->dwbm_addr_o
#define dwbm_dat_o  m_core->dwbm_dat_o
#define dwbm_sel_o  m_core->dwbm_sel_o
#define dwbm_cyc_o  m_core->dwbm_cyc_o
#define dwbm_stb_o  m_core->dwbm_stb_o
#define dwbm_we_o   m_core->dwbm_we_o
...
// -----------------------------------------------------------------------------
// The testbench
class MIRFAKTB: public Testbench<VMirfak> {
private:
        uint32_t m_exitCode;
public:
        // -----------------------------------------------------------------------------
        // Testbench constructor
        MIRFAKTB(double frequency, double timescale=1e-9): Testbench(frequency, timescale), m_exitCode(-1) {}
        // -----------------------------------------------------------------------------
        // Print exit message
        ...
        bool CheckTOHOST(WBMEMORY &memory, bool &ok) const {
                ...
                const bool isTHWrite = dwbm_cyc_o and dwbm_stb_o and (dwbm_addr_o == TOHOST) and dwbm_we_o and dwbm_ack_i;
                bool       _exit     = false;
                if (isTHWrite) {
                        bool isPtr = (dwbm_dat_o - MEMSTART) <= MEMSZ;
                        _exit      = dwbm_dat_o == 1 || not isPtr;
                        ok         = dwbm_dat_o == 1;
                        if (not _exit) {
                                const uint32_t data0 = dwbm_dat_o >> 2; // byte2word
        ...
        void IOtimeout() {
                // TODO: Add timeout to IWBM too...
                const uint32_t MAX_TIMEOUT  = 256;
                static uint32_t timeout_cnt = 0;

                if (dwbm_cyc_o and dwbm_stb_o) {
                        if (timeout_cnt++ == MAX_TIMEOUT and not (dwbm_ack_i or dwbm_err_i)) {
                                dwbm_err_i = true;
                                fprintf(stderr, ANSI_COLOR_RED "[MIRFAKTB] Access to unimplemented address: 0x%08X. Timeout: %d\n" ANSI_COLOR_RESET, dwbm_addr_o, timeout_cnt);
                        }
                } else {
                        timeout_cnt = 0;
                }
        ...
        int SimulateCore(const std::string &progfile, const unsigned long max_time=1000000L) {
                ...
                for (; getTime() < max_time;) {
                        Tick();
                        iwbm_ack_i = false;  // set default state for ack/err: IPORT
                        iwbm_err_i = false;
                        dwbm_ack_i = false;  // set default state for ack/err: DPORT
                        dwbm_err_i = false;

                        if (dwbm_cyc_o and not ip_bus) {
                                memory(dwbm_addr_o, dwbm_dat_o, dwbm_sel_o, dwbm_cyc_o, dwbm_stb_o, dwbm_we_o,
                                       dwbm_dat_i, dwbm_ack_i, dwbm_err_i);
                                if (CheckTOHOST(memory, ok)) {
                                        m_exitCode = dwbm_dat_o;
                                        break;
                                }
                                console(dwbm_addr_o, dwbm_dat_o, dwbm_sel_o, dwbm_cyc_o, dwbm_stb_o, dwbm_we_o,
                                        dwbm_dat_i, dwbm_ack_i, dwbm_err_i);
                                devint(dwbm_addr_o, dwbm_dat_o, dwbm_sel_o, dwbm_cyc_o, dwbm_stb_o, dwbm_we_o,
                                       dwbm_dat_i, dwbm_ack_i, dwbm_err_i,
...
int main(int argc, char **argv) {
        ...
        if (trace) {
                ...
                if (s_trace_name.empty()) {
                        ...
                        binfile = "/Mirfak_" + bf.substr(0, p) + ".vcd";
                } else {
